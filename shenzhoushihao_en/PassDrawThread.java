...
/*
 * 此类属于典型的DrawThread类，与本游戏中的WelcomeDrawThread和TouchThread类完全一样，实现的功能也是开启一个线程后，通过
 * 调用doDraw方法不断重回整个屏幕，由于牵涉到不同的View，线程的启动和停止，为了方便编程，所以这才没把这三个合并
 *
 */

public class PassDrawThread extends Thread                                     //继承Thread
{                      
	PassView father;                                                           //GameView对象引用
	SurfaceHolder surfaceHolder;                                               //SurfaceHolder对象引用
	boolean flag;                                                              //循环控制变量
	boolean isDrawOn;                                                          //内层循环变量
	
	public PassDrawThread(PassView father,SurfaceHolder surfaceHolder)         //构造方法
	{          
		super.setName("==PassDrawThread");                                     //初始化父类
