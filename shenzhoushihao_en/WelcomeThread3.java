...
public class WelcomeThread3 extends Thread                        //继承了Thread类 
{                          
	@SuppressWarnings("unchecked")
	WelcomeView3 father;                                          //WelcomeView3的对象引用
	boolean flag;                                                 //线程执行标志
	...
	{                          
		while(flag)                                               //标志为true时
		{                     
			switch(father.status)                                 //switch  WelcomeView3中的status 
			{         
				case 0:                                           //正在显示闪屏图片
					father.alpha +=2;                             //逐渐变暗消失
					if(father.alpha >=254)
					{                                             //完全消失时
						father.status =8;                         //跳到WelcomeView3中的显示声音开关选择界面
				...
				case 3:                                           //显示文字
					...
					{                                             //等于三时
						...
						father.characterNumber++;                 //WelcomeView3中显示的字数加1
						charNumber++;                             //写字计数器加1
						if(charNumber == father.str.length)
						{	  //字数全部显示完成时			
							father.status = 7;                    //跳到WelconeView3的case 7，绘制主功能界面
						}					
					}
					break;
				default:
